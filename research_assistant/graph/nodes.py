        ...
        if query.document_ids:
            for doc_id in query.document_ids:
                if doc_id not in state.documents:
                    ...
                    state.messages.append(AIMessage(content=f"Processed document {doc_id}"))
                    ...
                    state.messages.append(AIMessage(content=f"Marked document {doc_id} as processed"))
        else:
            ...
            if potential_docs:
                ...
                state.messages.append(AIMessage(content=f"Processed documents: {', '.join(potential_docs)}"))
                state.query.document_ids = potential_docs
            else:
                state.messages.append(AIMessage(content="No document references found in the query."))
...
def retrieve_information(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available to retrieve information from."))
        ...
        try:
            ...
            if not chunks:
                state.messages.append(AIMessage(content="No relevant information found in the documents."))
                return {"state": state, "next": "route"}
            ...
            state.messages.append(AIMessage(content=f"Retrieved {len(chunks)} relevant chunks from documents: {', '.join(doc_ids)}"))
            ...
            state.messages.append(AIMessage(content=f"Error retrieving information: {str(e)}"))
            return {"state": state, "next": "route"}
...
def generate_summary(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available to summarize."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Generated {summary_type} summaries for documents: {', '.join(doc_ids)}"))
            
        except Exception as e:
            state.error = f"Error generating summaries: {str(e)}"
            state.messages.append(AIMessage(content=f"Error generating summaries: {str(e)}"))
...
def extract_methodology(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available for methodology extraction."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Extracted methodology information from documents: {', '.join(doc_ids)}"))
            
        except Exception as e:
            state.error = f"Error extracting methodologies: {str(e)}"
            state.messages.append(AIMessage(content=f"Error extracting methodologies: {str(e)}"))
...
def extract_claims(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available for claim extraction."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            # Add message about extraction
            state.messages.append(AIMessage(content=f"Extracted key claims from documents: {', '.join(doc_ids)}"))
            
        except Exception as e:
            state.error = f"Error extracting claims: {str(e)}"
            state.messages.append(AIMessage(content=f"Error extracting claims: {str(e)}"))
...
def compare_documents(state: AgentState, tools: AgentTools):
        ...
        if not state.documents or len(state.documents) < 2:
            state.messages.append(AIMessage(content="Need at least two documents for comparison."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Compared documents: {', '.join(doc_ids)}"))
            
        except Exception as e:
            state.error = f"Error comparing documents: {str(e)}"
            state.messages.append(AIMessage(content=f"Error comparing documents: {str(e)}"))
...
def generate_citation(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available for citation generation."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Generated {style} citations for documents: {', '.join(doc_ids)}"))
            
        except Exception as e:
            state.error = f"Error generating citations: {str(e)}"
            state.messages.append(AIMessage(content=f"Error generating citations: {str(e)}"))
...
def answer_question(state: AgentState, tools: AgentTools):
        ...
        if not state.documents:
            state.messages.append(AIMessage(content="No documents available to answer questions from."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Answer: {answer}"))
            
        except Exception as e:
            state.error = f"Error answering question: {str(e)}"
            state.messages.append(AIMessage(content=f"Error answering question: {str(e)}"))
...
def generate_literature_review(state: AgentState, tools: AgentTools):
        ...
        if not state.documents or len(state.documents) < 2:
            state.messages.append(AIMessage(content="Need at least two documents for a literature review."))
            return {"state": state, "next": "route"}
        ...
        try:
            ...
            state.messages.append(AIMessage(content=f"Generated literature review for {len(doc_ids)} documents."))
            
        except Exception as e:
            state.error = f"Error generating literature review: {str(e)}"
            state.messages.append(AIMessage(content=f"Error generating literature review: {str(e)}"))
